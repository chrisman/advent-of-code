...
const sumOfAllDistances = ([ w, h, points, centers ]) => [
  w, h,
  points.map(point => [
    point[0],
    point[1].map(([ distance, _ ]) => distance).reduce((a, b) => a + b)
  ]),
  centers,
]

const belowThreshold = ([ w, h, points, centers ]) => [
  w, h,
  points.filter(point => point[1] < 10000),
  centers,
]

const regionSize =  ([ w, h, points, centers ]) => points.length
